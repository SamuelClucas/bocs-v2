...
impl GraphicsContext {
    pub fn new(win: Arc<Window>) -> Result<Self, Box<dyn Error>> {
        ...
        // async_setup: impl Future<Output = Result<(Adapter, Device, Queue), Box<dyn Error>>>
        let async_setup = async {
            let adapter = instance.request_adapter(&wgpu::RequestAdapterOptions {
                power_preference: wgpu::PowerPreference::default(),
                compatible_surface: Some(&surface),
                force_fallback_adapter: false
            }).await.ok_or("Failed to find a suitable GPU adapter")?; // returns option, so convert here to Result
        ...
        let (adapter, device, queue) = futures::executor::block_on(async_setup)?;
